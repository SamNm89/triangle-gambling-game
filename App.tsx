...
const App: React.FC = () => {
  ...
  const WalletDisplay = ({ className }: { className?: string }) => (
    <div className={`bg-[#0f212e] px-4 py-2 rounded-lg border border-[#2f4553] flex items-center justify-between gap-3 shadow-inner ${className}`}>
        ...
        {balance < betAmount && balance < 10 && (
             <button onClick={resetFunds} className="bg-[#2f4553] hover:bg-[#3f5563] p-1.5 rounded text-white transition animate-pulse">
                <RefreshCw size={14} />
            </button>
        )}
  ...
  return (
    <div className="h-full w-full flex flex-col md:flex-row bg-[#0f212e] text-white overflow-hidden font-sans select-none">
      ...
      <div className="order-2 md:order-1 w-full md:w-[320px] bg-[#1a2c38] flex flex-col border-t md:border-t-0 md:border-r border-[#263a4d] z-20 md:h-full shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.3)] md:shadow-none">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-5 flex flex-col gap-4">
            ...
            <div className="bg-[#213743] p-3 md:p-4 rounded-xl border border-[#2f4553] flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs text-gray-400 font-bold uppercase">Risk</label>
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-xs text-gray-400 font-bold uppercase">Rows</label>
                        <div className="relative">
                            <select 
                                value={rowCount} 
                ...
                <button 
                    ...
                    className={`w-full py-3.5 rounded-lg text-lg font-black uppercase tracking-wide shadow-lg transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 ${
                        balance < betAmount 
      ...
      <div className="order-1 md:order-2 flex-1 relative flex flex-col min-h-0 bg-[#0f212e]">
        ...
        <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 items-end pointer-events-none max-h-[50%] overflow-hidden mask-linear-fade">
            {history.slice(0, window.innerWidth < 768 ? 4 : MAX_HISTORY).map((h, i) => (
                <div key={i} className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-md shadow-lg border border-opacity-10 backdrop-blur-sm animate-in fade-in slide-in-from-right-8 duration-300 ${
                    h.profit > 0 
